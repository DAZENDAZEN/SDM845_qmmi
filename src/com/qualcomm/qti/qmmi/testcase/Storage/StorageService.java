...
public class StorageService extends BaseService {
    ...
    public int stop(TestCase testCase) {
		String sdPath = getSdDirectory();
		String CMD = "rm -f " + sdPath + "/test";
		LogUtils.loge("CMD" + CMD);

		String[] commands  = new String[]{CMD};
		ShellUtils.CommandResult copy = ShellUtils.execCommand(commands, false);
    ...
    public int run(TestCase testCase) {
        ...
        if(type.equals("sdcard")) {
            if (isSdMounted()) {
				...
				String sdPath = getSdDirectory();
				LogUtils.loge("sdPath" + sdPath);
				String CMD = "touch " + sdPath + "/test";
				LogUtils.loge("CMD" + CMD);

				String[] commands  = new String[]{CMD};
				ShellUtils.CommandResult copy = ShellUtils.execCommand(commands, false);

				String sdcardcopyresult="fail";
				if(copy.result == 0){
					sdcardcopyresult="true";
				}else{
					sdcardcopyresult="fail";
				}
                ...
                sb.append(this.getResources().getString(R.string.sdcard_total))
                    ...
                    .append(this.getResources().getString(R.string.sdcard_copy))
                    .append(sdcardcopyresult).append("\n");

                testCase.addTestData("Sdcard Total Size", sdcardTotalSize);
                testCase.addTestData("Sdcard Available Size", sdcardAvailSize);
				testCase.addTestData("Sdcard read/write : ", sdcardcopyresult);
