...
import com.qualcomm.qti.qmmi.testcase.Lcd.RawToBitMap;
...
*/
/**********************Tof test *********************************************/
		LogUtils.logi("Tof service run");

		String[] commands1  = new String[]{"tofhwtest 0 1 0 10"};
		ShellUtils.CommandResult tofservice1 = ShellUtils.execCommand(commands1, false);
		LogUtils.loge("TOF: " + tofservice1.toString());
		if(tofservice1.result == 0)
			{
			LogUtils.logi("tof_client_test ok");

			}else
			{
			LogUtils.logi("tof_client_test fail");
			}
		
		 new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				LogUtils.logi( "tof in!");
				try {
			        FileInputStream inputStream = new FileInputStream("/sdcard/sunny/raw/tof_original_000000.raw");
					Bitmap bitmap = RawToBitMap.convert8bit(inputStream, 224, 860);
